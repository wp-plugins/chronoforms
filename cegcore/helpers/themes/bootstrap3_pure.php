...
class Bootstrap3Pure extends \GCore\Helpers\Theme {
	var $view;
	...
	public static function html_formSecStart($class = '', $id = ''){ //bspure fix
	...
	public static function html_formLine($name, $params = array()){
		...
		$tags[] = \GCore\Helpers\Html::container('div', $input, array('class' => 'form-group', 'id' => $id)); //bspure fix
		return implode("\n", $tags);
	}

	public static function html_formInput($name, $params = array(), $formInputParams = array()){
		$class = (!empty($params['sub_input'])? 'gcore-sub-input ' : '').'gcore-display-table'; //bspure fix
