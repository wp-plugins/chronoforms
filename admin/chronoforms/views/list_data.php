			...
			<form action="<?php echo r_('index.php?ext=chronoforms&act=list_data&table='.$this->data['table']); ?>" method="post" name="admin_form" id="admin_form">
				<?php
					...
					foreach($columns as $column => $title){
						...
						if(empty($count)){
							$listing_row['ListData.'.$column]['link'] = r_('index.php?ext=chronoforms&act=show_data&table='.$this->data['table'].'&'.$pkey.'={ListData.'.$pkey.'}'.'&form_id='.$this->data['form_id']);
						}
						$count++;
					}
					$this->DataTable->cells($rows, array_merge(array(
						'CHECK' => array(
							'style' => array('width' => '5%'),
							'html' => $this->Toolbar->selector('{ListData.'.$pkey.'}')
						)), $listing_row)
					);
